...
2. Mode 2 fetch: `GET /v1/requests/{id}` is an idempotent, non-destructive read, with an optional DELETE endpoint for eager cleanup. The full TTL covers the client-paced window between completion and first fetch. After a delivered fetch the frontend shrinks the key's TTL to a short grace window, long enough for retries of a response lost in transit, since only the client can confirm receipt. Mode 3 wait: the POST handler subscribes to the key's arrival notification on the replica's shared pub/sub connection, checks the key once to close the arrival race, and parks until notification, backup poll tick, or the capped timeout (which returns the 202 response). Delivering on the held connection deletes the key outright, as a wait result has no other consumer (disconnected waiters are cancelled, not refetched). If the frontend fails to write a response, the result stays under its key and a re-fetch finds it.
